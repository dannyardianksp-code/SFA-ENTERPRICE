...
export default function CustomerGroupsPage() {
    ...
    const fetchGroups = async () => {
        ...
        const res = await fetch(`${API_BASE_URL}/customer-groups`, {
    ...
    const groupsHalamanIni = filteredGroups.slice(
    ...
    const handleDelete = async (g: any) => {
        ...
        if (!confirm(`Hapus customer group "${g.name}"? Tindakan ini tidak bisa dibatalkan.`)) {
            return
        }

        const token = localStorage.getItem('token')

        const res = await fetch(`${API_BASE_URL}/customer-groups/${g.id}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` }
        })
        ...
        if (!res.ok) {
            alert(data.message || 'Gagal menghapus customer group')
            return
        }
    ...
    const handleSubmit = async (e: any) => {
        ...
        const url = editId
            ? `${API_BASE_URL}/customer-groups/${editId}`
        ...
        if (!res.ok) {
            alert(data.message || 'Gagal menyimpan customer group')
            return
        }

        alert(editId ? 'Customer group berhasil diupdate' : 'Customer group berhasil ditambahkan')
        ...
        <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-lg">
                <h1 className="text-3xl font-bold text-slate-900">
                    🗂️ Customer Group Management
                </h1>
                <p className="text-slate-500 mt-1">
                    Kelompok customer -- dipakai saat membuat/mengedit data customer.
                </p>
            </div>
            ...
            {isAdmin && (
                <div className="bg-white rounded-3xl p-6 shadow-lg space-y-4">
                    ...
                    <h2 className="font-bold text-slate-900">
                        {editId ? '✏️ Edit Customer Group' : '➕ Tambah Customer Group Baru'}
                    </h2>
                    ...
                    <form onSubmit={handleSubmit} className="grid md:grid-cols-3 gap-4 items-end">
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {saving ? 'Menyimpan...' : editId ? 'Update' : 'Tambah Customer Group'}
                            </button>
                    ...
                    <tbody>
                        ...
                        {groupsHalamanIni.map((g: any) => (
                            ...
                            >
                                ...
                                <td className="p-4 text-slate-600">{g.code}</td>

                                <td className="p-4 font-semibold text-slate-900">{g.name}</td>
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-between p-4 border-t border-slate-200">
                        ...
                        <span className="text-sm text-slate-500">
                            Halaman {halamanAman} dari {totalPages} ({filteredGroups.length} customer group)
                        </span>
